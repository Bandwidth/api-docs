...
| message.media         | `array`  | A list of URLs to include as media attachments as part of the message.  Using the [v2 media api](../methods/media/about.md) you can download the media **WITHIN 2 DAYS**  The URL will look something like:  `https://messaging.bandwidth.com/api/v2/users/{accountId}/media/{uploaded-file-name}`  Where `filename` is the original filename of your uploaded media.     |

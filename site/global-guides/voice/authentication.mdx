...
The process to retrieve an access token requires you to have your [WebRTC Credentials](./inAppGuide.mdx#generating-credentials-for-an-access-token) (ie. ClientID and ClientSecret), generated from the [Global Portal](https://login.voxbone.com/login). You will then be able to make a call to our token endpoint with these credentials. This will return to you an `access_token`, `access_token_id`, `token_type`, and `expires_in` (this is the number of seconds before the token will expire).
...
sequenceDiagram
    ...
    In-App Calling Auth Server->>In-App Calling Auth Server: Validate credentials
    In-App Calling Auth Server->>Application: 200 OK <br> {"access_token": "...", "access_token_id": "...", "token_type": "Bearer", "expires_in": 600}
...
- **Headers**
  ...
  - **Content-Type** - For this request we **MUST** to use `application/x-www-form-urlencoded`. This will URL encode (aka percent-encode) our parameters making them into `key=value` pairs which will be separated by an ampersand `&`.
...
Once you have obtained the token you can perform authenticated requests to the WebRTC Gateway. All you need to do is add an `Authorization` header to the request with a value of `<token_type> <access_token>`.

Using the example response above a request would look similar to:
```sh
curl --request GET \
  --url https://<webrtc_gateway>/some/protected/path \
  --header 'Authorization: Bearer eyJraWQiOiJzZ25tLTk1ZTE0YTFiLTRiODItNDM5Zi1hYjhmLTYwMmI5MzYwMjZkZiIsImFsZyI6IlJTMjU2In0..kl3HYFQIcP4xZd78wUkdXxntxBu1d7b5ZCt99qVObC-gUZd5KSq2J7Q5rDb2LjP1_WVndcLQSqCoq3Anvp03hJWM6mamZL3dWHxjGLwkrIlzmNx9ZFGhTGIEsYLyaQqy8MonWYw2mJ4Z0APEfTVbTBHNIGrm_9GT6rIkdOwQFM-XgH1Tau4JbpFJun3n6o15WTgjzdEj9fIwd385CPwL-pAmOiozbYWUEzgqkXjSGHI11hiLDu-tv_8Ds06Cx4iCnL1F6_dFrnpD3CF0i6JJYVrvLmi6vgzoxp9YEIRBwaOZTSuYuYt03SQfbMZy8L2Z71sRKLLGt3TrxgtwyjefpQ'
```

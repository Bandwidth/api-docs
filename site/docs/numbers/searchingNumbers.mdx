...
::tip
The `tollFreeWildCardPattern` allows you to search toll free area codes, but forces the 3rd digit to be a `*`. However, with the nature of Toll Free area codes - the 3rd digit will always be a repeat of the second, so to search for 833 numbers, set the parameter value as `83*` and you will only return 833 numbers. For 800 numbers, search for `80*` and so on.
:::

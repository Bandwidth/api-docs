...
To see specific examples of each order type, please visit the [Dashboard API Reference](../../../numbers-api-reference).
...
**Note:** This is a webhook that bandwidth sends to _your_ server upon order completion/failure - there is no need to poll the resource if using this recommended method.

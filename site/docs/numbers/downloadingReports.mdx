...
Once the ID of the report you are looking to generate is ascertained, A <Highlight color="#079CEE">POST</Highlight> request to <code>https://dashboard.bandwidth.com/api/accounts/{accountId}/reports/{reportId}/instances</code> will trigger the generation of a new report and return a `201` response with a location header containing a URL used to query the status of the report.

---
id: lookupNumbers
title: How to use phone number lookup
slug: /numbers/guides/lookupNumbers
description: How to look up external numbers using the Bandwidth API
keywords:
  - bandwidth
  - numbers
  - lookup
  - request
image: '@site/static/img/bw-icon.svg'
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

export const accountId = "{accountId}";
export const orderId = "{orderId}";

export const Highlight = ({children, color}) => (
    <span
      style={{
        backgroundColor: color,
        borderRadius: '2px',
        color: '#fff',
        padding: '0.2rem',
      }}>
      {children}
    </span>
  );

A guide that walks through the Bandwidth API to provide carrier information for a telephone number or batch of telephone numbers.

Currently, this service supports lookups of telephone numbers in the mainland United States, Alaska, Hawaii, District of Columbia, and the provinces of Canada. Telephone numbers submitted must be in E.164 format to be processed.

## Submit Number Lookup Order

To submit number Lookup request, make a <Highlight color="#079CEE">POST</Highlight> request to our [Create Lookup Order API](../../../../apis/number-lookup#tag/Phone-Number-Lookup/operation/createLookup) endpoint.

<Tabs
  groupId="http_number_lookup_order"
  defaultValue="single_tn"
  values={[
    { label: 'Single Number Lookup', value: 'single_tn', },
    { label: 'Multiple Number Lookup', value: 'multiple_tn', },
  ]
}>
<TabItem value="single_tn">

Use one of the examples to create Single Number Lookup request. This can be done through tools like [Postman](https://github.com/Bandwidth/postman) or cURL.

#### Examples:

<Tabs
groupId="lookup_numbers"
defaultValue="payload"
values={[
    { label: 'Payload', value: 'payload', },
    { label: 'cURL', value: 'curl', }
]
}>

<TabItem value="payload">

<p><Highlight color="#079CEE">POST</Highlight> <code>https://numbers.bandwidth.com/api/v1/accounts/{accountId}/tnlookup</code></p>

```json
{
    "tns": [
        "19196104423"
    ]
}
```
</TabItem>

<TabItem value="curl">

```cURL
curl -X POST 'https://numbers.bandwidth.com/api/v1/accounts/{accountId}/tnlookup'
-U '{userName}:{password}'
-H 'Content-Type: application/json'
-d '{
        "tns": [
            "19196104423"
        ]
    }'
```
</TabItem>
</Tabs>

> Response

```json
HTTP/1.1 202 Accepted
Content-Type: application/json

{
    "requestId": "004223a0-8b17-41b1-bf81-20732adf5590",
    "status": "IN_PROGRESS"
}
```

</TabItem>
<TabItem value="multiple_tn">

Use one of the examples to create Multiple Number Lookup request. This can be done through tools like [Postman](https://github.com/Bandwidth/postman) or cURL.

#### Examples:

<Tabs
groupId="lookup_numbers"
defaultValue="payload"
values={[
    { label: 'Payload', value: 'payload', },
    { label: 'cURL', value: 'curl', }
]
}>

<TabItem value="payload">

<p><Highlight color="#079CEE">POST</Highlight> <code>https://numbers.bandwidth.com/api/v1/accounts/{accountId}/tnlookup</code></p>

```json
{
    "tns": [
        "19196104423",
        "+19196104424"
    ]
}
```
</TabItem>

<TabItem value="curl">

```cURL
curl -X POST 'https://numbers.bandwidth.com/api/v1/accounts/{accountId}/tnlookup'
-U '{userName}:{password}'
-H 'Content-Type: application/json'
-d '{
        "tns": [
            "19196104423",
            "+19196104424"
        ]
    }'
```
</TabItem>
</Tabs>

> Response

```json
HTTP/1.1 202 Accepted
Content-Type: application/json

{
  "requestId": "004223a0-8b17-41b1-bf81-20732adf5590",
  "status": "IN_PROGRESS"
}
```

</TabItem>
</Tabs>

## Fetch Order Information

Since the TN Lookup request is asynchronous it means that you need to make a <Highlight color="#00bf8c">GET</Highlight> request
to our [Get Lookup Order Status API](../../../../apis/number-lookup#tag/Phone-Number-Lookup/operation/getLookupStatus) endpoint to get the request status.
This can be done through tools like [Postman](https://github.com/Bandwidth/postman) or cURL.


### Examples:

#### Request URL

<Highlight color="#00bf8c">GET</Highlight> <code>https://numbers.bandwidth.com/api/v1/accounts/{accountId}/tnlookup/{orderId}</code>


<Tabs
groupId="numbers_lookup"
defaultValue="curl"
values={[
    { label: 'cURL', value: 'curl', }
]
}>

<TabItem value="curl">

```cURL
curl 'https://numbers.bandwidth.com/api/v1/accounts/{accountId}/tnlookup/{orderId}'
-U '{userName}:{password}'
```
</TabItem>
</Tabs>

#### Response statuses

The request status can be in one of these examples:

<Tabs
  groupId="http_number_lookup_order_status"
  defaultValue="in_progress"
  values={[
    { label: 'IN PROGRESS', value: 'in_progress', },
    { label: 'One TN - COMPLETE', value: 'one_tn_complete', },
    { label: 'Multiple TNs - COMPLETE', value: 'multiple_tn_complete', },
    { label: 'Multiple TNs - PARTIAL_COMPLETE', value: 'multiple_tn_partial_complete', },
    { label: 'No Information - COMPLETE', value: 'complete_no_info', },
    { label: 'FAILED', value: 'failed', },
  ]
}>
<TabItem value="in_progress">


```json
HTTP/1.1 200 OK
Content-Type: application/json

{
  "requestId": "004223a0-8b17-41b1-bf81-20732adf5590",
  "status": "IN_PROGRESS"
}
```

</TabItem>
<TabItem value="one_tn_complete">

```json
HTTP/1.1 200 OK
Content-Type: application/json

{
  "requestId": "004223a0-8b17-41b1-bf81-20732adf5590",
  "status": "COMPLETE",
  "result": [
    {
      "Response Code": 0,
      "Message": "NOERROR",
      "E.164 Format": "19196104424",
      "Formatted": "(919) 610-4424",
      "Country": "US",
      "Line Type": "Mobile",
      "Line Provider": "T-Mobile USA",
      "Mobile Country Code": "310",
      "Mobile Network Code": "160"
    }
  ]
}
```

</TabItem>
<TabItem value="multiple_tn_complete">

```json
HTTP/1.1 200 OK
Content-Type: application/json

{
  "requestId": "004223a0-8b17-41b1-bf81-20732adf5590",
  "status": "COMPLETE",
  "result": [
    {
      "Response Code": 0,
      "Message": "NOERROR",
      "E.164 Format": "+19196104424",
      "Formatted": "(919) 610-4424",
      "Country": "US",
      "Line Type": "Mobile",
      "Line Provider": "T-Mobile USA",
      "Mobile Country Code": "310",
      "Mobile Network Code": "160"
    },
    {
      "Response Code": 0,
      "Message": "NOERROR",
      "E.164 Format": "19196104423",
      "Formatted": "(919) 610-4423",
      "Country": "US",
      "Line Type": "Mobile",
      "Line Provider": "Verizon Wireless",
      "Mobile Country Code": "310",
      "Mobile Network Code": "010"
    }
  ]
}
```

</TabItem>
<TabItem value="multiple_tn_partial_complete">

```json
HTTP/1.1 200 OK
Content-Type: application/json

{
  "requestId": "004223a0-8b17-41b1-bf81-20732adf5590",
  "status": "PARTIAL_COMPLETE",
  "result": [
    {
      "Response Code": 0,
      "Message": "NOERROR",
      "E.164 Format": "+19196104424",
      "Formatted": "(919) 610-4424",
      "Country": "US",
      "Line Type": "Mobile",
      "Line Provider": "T-Mobile USA",
      "Mobile Country Code": "310",
      "Mobile Network Code": "160"
    }
  ],
  "failedTelephoneNumbers": [
    "+13992077164"
  ]
}
```

</TabItem>
<TabItem value="complete_no_info">

```json
HTTP/1.1 200 OK
Content-Type: application/json

{
  "requestId": "004223a0-8b17-41b1-bf81-20732adf5590",
  "status": "COMPLETE",
  "result": [
    {
      "Response Code": 3,
      "Message": "NXDOMAIN",
      "E.164 Format": "19196104425",
      "Formatted": "(919) 610-4425",
      "Country": "US"
    }
  ]
}
```

</TabItem>
<TabItem value="failed">

```json
HTTP/1.1 200 OK
Content-Type: application/json

{
  "requestId": "004223a0-8b17-41b1-bf81-20732adf5590",
  "status": "FAILED",
  "failedTelephoneNumbers": [
    "+13992077164"
  ]
}
```

</TabItem>
</Tabs>


## Where to next?
Now that you have learned how to lookup for numbers, check out some of the other available actions in our guides:
- [How to port numbers](/docs/numbers/guides/porting/portingNumbers)
- [How to order numbers](/docs/numbers/guides/searchingForNumbers)
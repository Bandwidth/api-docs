...
At a given time, the request status for requests made can be 'IN PROGRESS', 'COMPLETE', or 'FAILED'.
For requests on multiple TNs, that status can also be 'PARTIAL_COMPLETE'. Please refer to the examples below to see possible responses:

...
Most telephone number features are controlled by the TnOptions order type. This API endpoint allows you to configure different settings for multiple groups of numbers in one API call. For ex: you can set LIDB information on one group of numbers and DLDA information on another, entirely different group of numbers, in the same API call.
...
| `SMS` | Optional | `on` _or_ `off` _or_ `unchanged` | Turn on/off SMS for a group of numbers |

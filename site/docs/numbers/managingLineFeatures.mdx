...
**NOTE:** These orders require manual intervention and take over 24 hours to process.
...
The <Highlight color="#00bf8c">GET</Highlight> method retrieves detailed information about the phone number.
...
```xml
<FinalDestinationURI>+13332221111@PSTN</FinalDestinationURI>
```
...
**NOTE** This PUT request is idempotent - it will overwrite the existing location (sip-peer) settings when made so be sure to include any previous settings that must persist on the location or you risk replacing them with their default `null` values

...
The messaging API works off of an internal queuing system.  As such, when you <Highlight color="#079CEE">[POST](/apis/messaging#operation/CreateMessage)</Highlight> to `/messages` to create a new message, Bandwidth will reply with an `HTTP 202 - Accepted`.  This indicates that the message has been placed in the queue.

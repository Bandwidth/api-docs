...
The sequence diagram below shows a hypothetical usage scenario.
  ...
  * Add some locations by calling the [addLocation](/dash-api-reference#operation/addLocation) method. You can have multiple locations for one URI. The maximum number of locations for one URI is configured in the system. Contact Bandwidth Support if you need to add more locations than the default three. Each call to [addLocation](/dash-api-reference#operation/addLocation) returns a Location that has status information so you can see if the location was GEOCODED, and an ID (locationId) that can be used later to uniquely provision or remove the location.
  * After calling [addLocation](/dash-api-reference#operation/addLocation) a few times, use [provisionLocation](/dash-api-reference#operation/provisionLocation) to set the active location. Send in the locationId to specify what location to provision out of the locations added to the URI. The [provisionLocation](/dash-api-reference#operation/provisionLocation) call will return another status. A successful call will return PROVISIONED status.
  ...
  * In this example, we remove a location by sending the locationId to [removeLocation](/dash-api-reference#operation/removeLocation). The results will come back with a status of REMOVED.
  ...
  * Finally, if the number is no longer used, call [removeURI](/dash-api-reference#operation/removeLocation) to unprovision any active location that is associated with the URI. The method will remove all the locations and the URI from the system.

...
For inbound calls, Bandwidth will also send [disconnect webhooks](./webhooks/disconnect.mdx) to the configured `CallStatusCallbackUrl`. This webhook is useful for error checking and include the cause such as _rejected_ and _hung up_.

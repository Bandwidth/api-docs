...
Every application requires a `CallInitiatedCallbackUrl`, as this is the URL Bandwidth sends an [initiated webhook](/docs/voice/webhooks/initiate/) to when an inbound call to a Bandwidth number is made,
and a BXML response is expected to continue the call.

For inbound calls, Bandwidth will also send [disconnect webhooks](/docs/voice/webhooks/disconnect/) to the configured `CallStatusCallbackUrl`. This webhook is useful for error checking and includes the cause of the disconnect, for example _rejected_ or _hung up_.

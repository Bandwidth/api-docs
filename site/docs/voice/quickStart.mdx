...
For inbound calls, Bandwidth will also send [disconnect events](./webhooks/disconnect.mdx) to the configured `CallStatusCallbackUrl`. These webhooks are useful for error checking and include the cause such as _rejected_ and _hung up_.

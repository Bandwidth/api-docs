...
The audio file should already be hosted and the URL of an audio file should be included in the body of the [`<PlayAudio>`](/docs/voice/bxml/playAudio) tag. However, it may be a relative URL that is then hosted by your device.
...
The example above plays two audio files for the caller; one from an absolute endpoint hosted somewhere other than where the application is, and one is a relative endpoint. Note the relative endpoint needs to also be served by the application, and deliver the file.
...
The SpeakSentence verb is used for text-to-speech playback on a call. Attributes of the speaker may be changed including the gender or locale of the speaker. The default speaker *susan* is a female speaker with locale en_US. All supported speakers can be viewed [here](/docs/voice/bxml/speaksentence).

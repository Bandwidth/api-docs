...
We use a 15s request timeout by default when trying to deliver all s. However, you can customize that value [when creating an outbound call](/voice-api-reference#operation/createCall) by setting a `callbackTimeout` on the request; or with application settings for inbound calls.
...
| [Redirect](/docs/voice/webhooks/redirect)                           | Bandwidth API sends this when a `<Redirect>` is requested or when a <Highlight color="#079CEE">POST</Highlight> request is made to the [/calls/{callId}](/voice-api-reference#operation/modifyCall) endpoint requesting a redirect. |
...
| [Conference Redirect](/docs/voice/webhooks/conferenceRedirect)      | Bandwidth API sends this to the application when a <Highlight color="#079CEE">POST</Highlight> request is made to the [/conferences/{conferenceId}](/voice-api-reference#operation/modifyConferenc) endpoint requesting to modify a conference. |

...
One of the [Bridge Complete](/docs/voice/webhooks/bridgeComplete) or [Bridge Target Complete](/docs/voice/webhooks/bridgeTargetComplete)
webhooks is sent when the bridge ends, to allow the call that remained in the bridge to execute new BXML. For example, if the call
that executed the Bridge verb hangs up or is [redirected](/apis/voice#operation/modifyCall) first, the
[Bridge Target Complete](/docs/voice/webhooks/bridgeTargetComplete) webhook is sent and the BXML returned from that will
be executed on the bridge target. In this case, the [Bridge Complete](/docs/voice/webhooks/bridgeComplete) webhook is NOT sent.
Similarly, if the target of the Bridge verb hangs up or is [redirected](/apis/voice#operation/modifyCall) first, the
[Bridge Complete](/docs/voice/webhooks/bridgeComplete) webhook is sent and the BXML returned from that will be executed
on the call that executed the Bridge. In this case, the [Bridge Target Complete](/docs/voice/webhooks/bridgeTargetComplete) webhook
is not sent.

...
When used on a call, audio from one or both sides of the call will be sent to the specified destination. The stream will continue until the call ends or the [`<StopStream>`][1] verb is used.
...
| track              | (optional) The segment of the call to send a stream for. `inbound`, `outbound` or `both`. Default is `inbound`.                                                                                                                    |
...
| streamEventsUrl    | (optional) URL to send the associated Webhook events to during this stream's lifetime. Does not accept BXML. May be a relative URL.                                         |
...
You may specify a number of `<CustomParam/>` elements nested within a `<StartStream>` tag. These elements define optional user specified parameters that will be sent to the destination URL when the stream is first started.
...
<Response>
    ...
    <StartStream name="live_audience" tracks="both" destination="wss://live-studio-audience.myapp.example.com" streamEventsUrl="https://myapp.example.com/noBXML">
        <CustomParam name="internal_id" value="call_ABC" />
    </StartStream>
...
CustomParam customParam = CustomParam.builder()
        .name("internal_id")
        .value("call_ABC")
        .build();
...
StartStream startStream = new StartStream
{
    ...
    Destination = "wss://live-studio-audience.myapp.example.com",
    StreamEventsUrl = "https://myapp.example.com/noBXML",
    CustomParams = new CustomParams[] { customParam }
};
...
custom_param = Bandwidth::Voice::CustomParam.new({
    :name => "internal_id",
    :value => "call_ABC"
})
...
$customParam = new BandwidthLib\Voice\Bxml\CustomParam();
$customParam->name("internal_id");
$customParam->value("call_ABC");
...
$startStream->customParams(array($customParam));
...
[2]: /doc/voice/webhooks/mediaStreamStarted
[3]: /doc/voice/webhooks/mediaStreamRejected
[4]: /doc/voice/webhooks/mediaStreamStopped

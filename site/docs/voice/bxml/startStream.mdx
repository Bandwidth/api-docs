...
When used on a call, audio from one or both sides of the call will be sent to the specified destination. The stream will continue until the call ends or the [`<StopStream>`][1] verb is used. A total of 4 concurrent track streams are allowed on a call. This can be two `<StartStream>` requests with `both` tracks, or four `<StartStream>` requests with either the `inbound` or `outbound` tracks, or any combination of those.

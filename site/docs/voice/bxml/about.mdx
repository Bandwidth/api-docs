...
The first BXML webhook is made when the call is created with [<Highlight color="#079CEE">POST</Highlight> /calls](/voice-api-reference#operation/createCall) as
...
| [Conference Redirect](/docs/voice/webhooks/conferenceRedirect)      | Bandwidth API sends this to the application when a <Highlight color="#079CEE">POST</Highlight> request is made to the [/conferences/{conferenceId}](/voice-api-reference#operation/modifyConference) endpoint requesting to modify a conference. |

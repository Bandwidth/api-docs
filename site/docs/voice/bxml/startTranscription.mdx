...
If the `transcriptionEventUrl` attribute is specified, then the [StartTranscription Available][5] event will be sent once the transcription has ended providing a URL from where the transcription can be downloaded. BXML returned in response to this callback will be ignored.

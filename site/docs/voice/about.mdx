...
A <Highlight color="#079CEE">[POST](/apis/voice#operation/createCall)</Highlight> to `/calls` creates a call object in Bandwidth's network and returns a promise in the form of a unique `callId`. When creating a call you provide an `answerUrl` in the request body, and upon answer Bandwidth will send a webhook to the provided URL confirming and expecting BXML in the HTTP response to continue with the call.

...
For help on how to make the proper requests to the API, check out the [API Reference](/apis/voice#operation/createCall) section of the docs. To quickly start making and receiving calls, check out the [guide](docs/voice/quickStart.mdx).
...
The Bandwidth API uses STIR/SHAKEN to help combat fraudulent calls and phone number spoofing. This info can be seen in the [`initiate`](/docs/voice/webhooks/initiate.mdx) webhook. For more information, look [here](https://www.bandwidth.com/regulations/stir-shaken/) for Bandwidth's explanation of what it is and why it's important.
...
### [Conferencing](/docs/voice/bxml/conference.mdx)

The Conference BXML Verb allows customers to create conference calls.

### [Answering Machine Detection](/docs/voice/machineDetectionGuide.mdx)

Machine Detection allows applications to distinguish whether a human or machine is speaking on a call.

### [Caller Name Service](/docs/voice/cnamPerDip.mdx) (CNAM)

Bandwidth's Caller Name service provides the calling name on VoIP calls in the US and Canada using our CNAM database. This feature must be enabled.

...
- If the [`gatherUrl`](/docs/voice/bxml/gather) attribute is specified, the Gather event is sent to the `gatherUrl` upon completion of the gather. BXML returned by that callback are then executed. If [`gatherUrl`](/docs/voice/bxml/gather) is specified, verbs following the [`<Gather>`](/docs/voice/bxml/gather) will be ignored.
...
In this example, there is one [`<SpeakSentence>`](/docs/voice/bxml/speakSentence) and two [`<PlayAudios>`](/docs/voice/bxml/PlayAudio) nested within the gather. If, after playing, the caller doesn't press a button, the audio will replay two more times.

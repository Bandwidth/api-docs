...
- If the `gatherUrl` attribute is specified, the Gather event is sent to the `gatherUrl` upon completion of the gather. BXML returned by that callback are then executed. If `gatherUrl` is specified, verbs following the [`<Gather>`](/docs/voice/bxml/gather) will be ignored.

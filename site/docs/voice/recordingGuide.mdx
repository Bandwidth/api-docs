...
<Response>
    ...
    <Record recordingAvailableUrl="https://myapp.com/recordingAvailable" maxDuration="10"/>
    <!-- Any verbs after this point would be replaced by those returned by the recordCompleteUrl -->
</Response>
```
/nextBXML responds with...
```xml
<Response>
...
Much like the Record verb, `<StartRecording>`(/docs/voice/bxml/startRecording) also has a `recordingAvailableUrl` attribute to receive the recording available callback.

...
If you want your recordings to be transcribed, you can add a `transcribe` attribute to your `Record` or `StartRecording` BXML verbs. If you want to receive a [TranscriptionAvailable](/docs/voice/webhooks/transcriptionAvailable) callback, you can additionally set the `transcriptionAvailableUrl` to a URL on your callback server and `transcriptionAvailableMethod` to its associated HTTP method (`GET` or `POST`, the default).

...
If you want to do a transcription of your recording, you can add a `transcribe` attribute to your `Record` or `StartRecording` BXML verbs. If you want to receive a [TranscriptionAvailable](/docs/voice/webhooks/transcriptionAvailable) callback, you can set the `transcriptionAvailableUrl` to your URL of choice, and `transcriptionAvailableMethod` to your HTTP method (`GET` or `POST`).

...
The first BXML webhook is made when the call is created with [POST /calls](/voice-api-reference#operation/createCall) as
...
| [Redirect](/docs/voice/webhooks/redirect)                           | Bandwidth API sends this when a `<Redirect>` is requested or when a POST request is made to the [/calls/{callId}](/voice-api-reference#operation/modifyCall) endpoint requesting a redirect.     |
...
| [Conference Redirect](/docs/voice/webhooks/conferenceRedirect)      | Bandwidth API sends this to the application when a POST request is made to the [/conferences/{conferenceId}](/voice-api-reference#operation/modifyConference) endpoint requesting to modify a conference. |

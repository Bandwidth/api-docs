...
| [Redirect](/docs/voice/webhooks/redirect)                           | Bandwidth API sends this when a `<Redirect>` is requested or when a POST request is made to the [/calls/{callId}](../methods/calls/postCallsCallId) endpoint requesting a redirect.     |
...
| [Conference Redirect](/docs/voice/webhooks/conferenceRedirect)      | Bandwidth API sends this to the application when a POST request is made to the [/conferences/{conferenceId}](../methods/conferences/postConferencesConferenceId) endpoint requesting to modify a conference. |

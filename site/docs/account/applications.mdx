...
In order to successfully use the Voice & Messaging APIs, you need to configure an `Application` for **both** the [Voice](../voice/about) & the [Messaging](../messaging/about) APIs.

Applications are where you configure webhook URLs for Bandwidth to send either [message](../../messaging/callbacks/messageEvents.md) or [voice](../../voice/bxml/callbacks/about.md) events to your service.
...
**Note:** Once the application `ServiceType` has been set - it can not be changed.
